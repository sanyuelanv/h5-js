...
/*
 2.
 2015年8月7日 11:46:38
 注意 占用html id两个：palette_canvas && palette 切勿冲突
 画板palette对象
 参数说明:
    {
         id: //标签在这个id的层级里面新建一个div，就是我们的画板层：由一个div和一个canvas组成
         size://整个画板层的大小，填写｛w：xxx,h:xxx｝
         pos://在页面的位置，以左上角为原点，填写｛x：xxx,y:xxx｝
         bg://画板canvas的颜色或者背景图。颜色必需用#XXXXXX的方式填入
         toucheSize://笔触大小
         color://笔触颜色
    }
 函数说明：
     setTouchSize(s) //传入更改后笔触大小即可
     setColor(c)     //传入更改后笔触颜色即可
     resetCanvas()  //重置画板
     creatPNG()    //返回一个imageURL
     createJPGE(p)    //传入质量参数(0~100)即可,返回一个imageURL
 */
...
var Palette = function(value){
    /*初始化数据*/
    var id =  value.id || -1;
    var size = value.size || {w:'200',h:'200'};
    var pos = value.pos || {x:'0',y:'0'} ;
    var surface_bg = value.bg || '#fff';
    var touchSize = value.touchSize || 60;
    var touchColor = value.color || '#000';
    /*画笔一些功能的初始化*/
    var canvas = undefined,ctxt = undefined,offset = {left:pos.x-1,top:pos.y-1},lines = [];
    var mouse = false;
    /*四个函数：画板大小，画笔颜色，画板重置，生成图片png/jpg */
    this.setTouchSize = function(s){ctxt.lineWidth = s;};
    this.setColor = function(c){ctxt.fillStyle = c;ctxt.strokeStyle=c;};
    this.resetCanvas = function(){
        ctxt.clearRect(0,0,size.w,size.h);
        if(surface_bg.indexOf('#')==-1){
            var img = new Image();
            img.src = surface_bg;
            img.onload = function() {ctxt.drawImage(img,0,0,size.w+2,size.h+2);obj.DrawIng();};
        }
        else{
            ctxt.fillStyle = surface_bg;
            ctxt.fillRect(0,0,size.w+2,size.h+2);
            obj.DrawIng();
        }
    };
    this.creatPNG = function(){var image = canvas.toDataURL("image/png");return image;};
    this.createJPGE = function(p){
        var dataURL;
        if(/Android (\d+\.\d+)/.test(navigator.userAgent)){
            var imgEncoder = new JPEGEncoder();
            dataURL = imgEncoder.encode(ctxt.getImageData(0, 0,canvas.width,canvas.height),p,true);
        }
        else{dataURL = canvas.toDataURL("image/jpeg",p/100);}
        return dataURL;
    };
    var obj = {
        init:function(){obj.create();},
        create:function(){
            if(id==-1){var dom = document.getElementsByTagName('body')[0]}else{var dom = document.getElementById(id);}
            var style = "width:"+size.w+"px;height:"+size.h+"px;position:absolute;left:"+pos.x+"px;top:"+pos.y+"px;";
            var str = "<div id='palette' style='"+style+"'><canvas id='palette_canvas' height='"+size.h+"' width='"+size.w+"'></canvas></div>";
            dom.innerHTML = dom.innerHTML + str;
            obj.creatCanvas();
        },
        creatCanvas:function(){
            canvas = document.getElementById('palette_canvas');
        ...
        DrawIng:function(){
            ...
            ctxt.fillStyle = touchColor;ctxt.strokeStyle=touchColor;
            /*监听touch事件或者click*/
